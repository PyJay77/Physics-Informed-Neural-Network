...
class NS(nn.Module):
    def __init__(self,
        ...
        self.sigma_x, self.sigma_y, self.sigma_t = fourier_features_parameters[0], fourier_features_parameters[1], fourier_features_parameters[2]
        ...
        self.t_max = self.T * self.U_max / self.D
        ...
        Wt = (torch.randn(size=(1,self.M))).mul(self.sigma_t)
    ...
    def forward(self, x, y, t):
        ...
        xx = 2*x.div(self.x_max) - 1
        yy = 2*y.div(self.y_max) - 1
        tt = 2*t.div(self.t_max) - 1
        ...
        xyt_mapped = torch.cat((torch.cos(Bx), torch.sin(Bx),
                                torch.cos(By), torch.sin(By),
                                torch.cos(Bt), torch.sin(Bt)), dim=1)         # N x (2 * number_input * M) 
